...
export function initAdmin(socket) {
    ...
    function renderItems(items) {
        ...
        return parsedItems.map(menuItem => {
            const name = menuItem.item.name || `Item: ${menuItem.item.item}, Size: ${menuItem.item.size}, Type: ${menuItem.item.type}, Extras: ${menuItem.item.extras}`;
            const qty = menuItem.qty || menuItem.item.quantity;
            return `<p>${name} - ${qty} pcs</p>`;
        }).join('');
    }
    ...
    function generateMarkup(orders) {
        return orders.map(order => {
            ...
            const totalPrice = Object.values(order.items).reduce((total, item) => {
                const price = item.item.price || item.customprice;
                const quantity = item.qty || item.item.quantity;
